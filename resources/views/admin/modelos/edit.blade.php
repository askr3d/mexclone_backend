...
@section('content_header')
    <h1>Editar Modelo: {{ $modelo->marca->nombre }} - {{ $modelo->nombre }}</h1>
...
@section('content')
    <main>
        <form
            action="{{ route('admin.modelos.update', $modelo->id) }}"
            method="POST"
            class="w-50 mx-auto">
            @csrf
            @method('PUT')
            <div class="form-group">
              ...
              <input type="text" class="form-control" name="modelo" id="modelo" placeholder="Modelo" value="{{ old('modelo', $modelo->nombre ) }}">
              @error('modelo')
            ...
            <div class="form-group">
                ...
                <select class="form-control" id="marca" name="marca">
                    ...
                    @foreach ($marcas as $marca)
                        <option value="{{ $marca->id }}" {{ $modelo->marca_id == $marca->id ? 'selected': '' }}>{{ $marca->nombre }}</option>
                    @endforeach
                </select>
                @error('marca')
                    <p class="text-danger" style="font-size: .9rem">{{ $message }}</p>
