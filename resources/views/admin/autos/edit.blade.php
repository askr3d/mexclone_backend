...
@section('content')
    <main class="py-4">
        <form
            action="{{ route('admin.autos.update', $auto->id) }}"
            method="POST"
            class="w-50 mx-auto">
            @csrf
            @method('PUT')
            <div class="row">
                <div x-data="{ placas: '{{ old('placas', $auto->placas) }}' }" class="form-group col-9">
                ...
                <div class="form-group col-3">
                    ...
                    <select class="form-control" name="estado" id="estado" value="{{ old('estado') }}">
                        ...
                        @foreach ($estados as $estado)
                            <option value="{{ $estado->id }}" {{ $auto->estado->id === $estado->id ? 'selected': '' }}>{{ $estado->nombre }}</option>
                        @endforeach
                    </select>
                    @error('estado')
                        <p class="text-danger" style="font-size: .9rem">{{ $message }}</p>
            ...
            <div class="row">
                <div class="form-group col-6">
                    ...
                    <input type="number" class="form-control" id="precio_por_dia" name="precio_por_dia" step="0.01" value='{{ old('precio_por_dia', $auto->precio_por_dia) }}' placeholder="Precio por día">
                ...
                <div class="form-group col-6">
                    ...
                    <input type="number" class="form-control" id="precio_total" name="precio_total" step="0.01" value='{{ old('precio_total', $auto->precio_total) }}' placeholder="Precio total">
                    @error('precio_total')
            ...
            <div class="row">
                <div class="col-5" x-data="{ showCiudades: '{{ old('pais', $auto->ciudad->pais->id) }}' }">
                ...
                <div class="col-2"></div>
                <div class="col-5" x-data="{ showModelos: '{{ old('marca', $auto->modelo->marca->id) }}' }">
...
@section('js')
    ...
    <script>
        ...
        const selectMarcaConfig= () => {
            ...
            const oldModelo = {!! json_encode(old('modelo', $auto->modelo->id)) !!};
            const oldMarca = {!! json_encode(old('marca', $auto->modelo->marca->id)) !!}
            console.log(oldModelo, oldMarca);
        ...
        const selectPaisConfig= () => {
            ...
            const oldCiudad = {!! json_encode(old('ciudad',$auto->ciudad->id)) !!};
            const oldPais = {!! json_encode(old('pais', $auto->ciudad->pais->id)) !!};
